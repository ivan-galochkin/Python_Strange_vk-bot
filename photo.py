...
def glitch_this_run(url, type, color='', lines=''):
    p = requests.get(url)
    out = open("glitch.jpg", "wb")
    out.write(p.content)
    out.close()
    command = f'glitch_this /usr/local/bin/evabot/glitch.jpg {type} {color} {lines}'
    os.system(command)
    os.remove('glitch.jpg')


def based(url):
    p = requests.get(url)
    out = open("imgbased.jpg", "wb")
    out.write(p.content)
    out.close()

    im = Image.open("imgbased.jpg")
    im = im.resize((1000, 1000))
    draw = ImageDraw.Draw(im)
    font = ImageFont.truetype("Impact", 400)
    draw.text((0, 590), "BASED", (255, 255, 255), font=font)
    im.save('imgbaseded.jpg')
    os.remove("imgbased.jpg")
